...
public class Validation {
    ...
    boolean isValidContactNo(String contactNo) {
        /*
    ...
    boolean isValidVehicleType(String vehicleType) {
        ...
        return vehicleType != null
                && (vehicleType.equals("mini") || vehicleType.equals("large") || vehicleType.equals("compact"));
    }

    boolean isVehicleNoValid(String vehicleNo) {
        /*
    ...
    int getCostByVehicleType(String vehicleType) {
        /*
    ...
    boolean isAdminSecretKeyValid(String secretKey) {
        // admin secret key is subsequence of actual secret key
        int n = secretKey.length();
        if (n < 40 || n > 60)
            return false; // Length must be between 40 and 60
    ...
    boolean isValidDateString(String dateStr) {
        /*
        ...
        if (stringToInteger(year) > currentYear || stringToInteger(year) < 2020)
            return false;
        if (stringToInteger(month) < 1 || stringToInteger(month) > 12)
            return false;
        if (stringToInteger(day) < 1 || stringToInteger(day) > 31)
            return false;
        if (!isMonthAndDayValid(stringToInteger(year), stringToInteger(month), stringToInteger(day)))
            return false;
        if (stringToInteger(hour) < 0 || stringToInteger(hour) > 23)
            return false;
        if (stringToInteger(minute) < 0 || stringToInteger(minute) > 59)
            return false;
        if (stringToInteger(second) < 0 || stringToInteger(second) > 59)
            return false;
        return true;
    }
